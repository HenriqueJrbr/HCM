...
//        die();
        ...
        $data['usuarios']    = isset($solicUsuarios['usuarios']) && count($solicUsuarios['usuarios']) > 0 ? $solicUsuarios['usuarios'] :  $solicUsuarios['solicitante'];
        $data['solicitante'] = $solicUsuarios['solicitante'];
        $data['acompanhante'] = $s->usuariosAcompanhantes();
        $data['idFluxo'] = 4;
        
        $this->loadTemplate('solicitacao_acesso_cadastro', $data);
    }
    ...
    public function criaSolicitacaoAcesso()
    {
        ...
        foreach ($rsUsuarios['dados'] as  $value):
            ...
            $rsGrupos = $s->dadosGestorGrupos($idProgramas, $idUsuario, $idEmpresa);
            ...
            foreach ($dadosGrupos as $key => $grupo):
                ...
                $dados = array(
                    ...
                    'manterStatus'      => (isset($_POST['manterStatus']) && $_POST['manterStatus'] == true) ? 1 : 0,
                    'obs'               => '',
                    'aprovacao'         => '',
                    'cartaRisco'       => '',
                    'obsGestorUsuario'  => '',
    ...
    {                
        ...
        if (isset($_POST['string']) && !empty($_POST['string'])) {
            $progs = $s->buscaProgramas($_POST['string'], $_POST['idUsuario'], (isset($_POST['progsJaAdd']) ? $_POST['progsJaAdd'] : []));
    ...
    public function enviaEmail($idGestor, $idUsuario, $mensagem = '')
    {
        ...
        if(empty($mensagem)):
            ...
            $mensagem = '
                ...
                <br><br>
                <span style="font-size:14px;margin-top:20px"><b>Atividade:</b> Solicitação de Acesso por Função do usuário <strong>{usuario}</strong>.</span>
                <br>
    ...
    public function ajaxValidaGrupoPrograma()
    {
        ...
        // Valida se usuário já possui solitição aberta com mesmo programa        
        // true = possui, false = não possui        
        $validaSolicitacaoAberta = $s->validaSolicitacaoAberta($_POST);
        
        if(!$validaSolicitacaoAberta):
        ...
        // Valida se o usuário já possui algum grupo com o programa solicitado
        $validaGrupoProg = $s->validaGrupoPrograma($_POST);
        if(isset($validaGrupoProg) && $validaGrupoProg['return']):
            echo json_encode(array(
                'return' => false,
    ...
    public function ajaxCarregaGruposProgramasAdd()
    {
        ...
        $grupos = $s->carregaGrupoProgramas($_POST['idPrograma'], $_POST['idUsuario']);
    ...
    public function ajaxMatrizDeRisco()
    {
        ...
        foreach ($dados['conflitos'] as $value) {
            ...
            if(isset($next['codRisco']) && $risco != $next['codRisco']){
                $html .='</tbody></table>';
            }


            if(isset($next['codRisco']) && $area != $next['descArea']){
                $html .='</div></div></div>';
