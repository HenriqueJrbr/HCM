...
class SolicitacaoAcessoGrupoController extends Controller
{
...
//        die();
        ...
        $this->loadTemplate('solicitacao_acesso_grupo_cadastro', $data);
    }
    ...
    public function criaSolicitacaoAcesso()
    {
        ...
        $idGrupos = [];
        foreach($_POST['grupo'] as $key => $val):
            array_push($idGrupos, $val['idGrupo']);
        endforeach;
        ...
        foreach ($rsUsuarios['dados'] as  $value):
            ...
            $paramsSolicitacao = array(
                ...
                'tipoEnvioMailCopia' => $_POST['tipoEnvioMailCopia'],
                'tipoFluxo'          => 'Solicitação de Acesso Por Grpuo',
                'dataMovimentacao'  => date('Y-m-d H:i:s')
            );
    ...
    public function criaProximaAtividade($criouMovimentacao, $params, $post)
    {
        ...
        if($criouMovimentacao == false):
            ...
            echo $objProxAtividade."<br>".$params['idProximaAtiv']."<br>";
            // Executa método responsável pela criação da movimentação
            if($objProxAtividade == 'criaAtividadeGestorModulo' || $objProxAtividade == 'criaAtividadeGestorRotina' || $objProxAtividade == 'criaAtividadeGestorPrograma'):
                $aprovador = lcfirst(str_replace('criaAtividade', '', $objProxAtividade));
    ...
    public function enviaEmail($idGestor, $idUsuario, $mensagem = '')
    {
        ...
        $nomeRemetente = "SGA - Sistema de Gestão de Acesso por Gruppo";
        ...
        if(empty($mensagem)):
            ...
            $mensagem = '
                ...
                <br><br>
                <span style="font-size:14px;margin-top:20px"><b>Atividade:</b> Solicitação de Acesso por Grupo do usuário <strong>{usuario}</strong>.</span>
                <br>
    ...
    public function enviaEmailCopia($paramsSolicitacao)
    {
        ...
        $f = new Fluxo();
        $assunto = "Solicitação de Acesso";
        $nomeRemetente = "SGA - Sistema de Gestão de Acesso por Grupo";
    ...
    public function ajaxValidaGrupo()
    {
        ...
        if(!isset($_POST['idGrupo']) || $_POST['idGrupo'] == '' || !isset($_POST['idUsuario']) || $_POST['idUsuario'] == ''):
            return '0';
        endif;                                   
    ...
    public function ajaxCarregaGruposProgramasAdd()
    {
        ...
        // Busca programas pelo id do grupo selecionado
        $grupos = $s->carregaGrupoProgramas($_POST['idGrupo'], $_POST['idUsuario']);
