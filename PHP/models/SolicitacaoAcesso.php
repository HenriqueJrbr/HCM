...
class SolicitacaoAcesso extends Model 
{
     ...
     * @return type
     */
    public function carregaGrupoProgramas($idProgramas, $idUsuario)
    {
        $sql = "
            SELECT
                gp.idPrograma,                
    ...
    /**
     * Busca se usuário já possui uma solicitação por determinado programa
     * @param type $post
     * @return type Array
     */
    public function validaSolicitacaoAberta($post)
    {
        ...
        if(isset($sql['idSolicitacao']) &&  $sql->rowCount() > 0):                                           
    ...
    /**
     * Retorna grupos pertencentes a usuário conforme id do programa e id do usuário
     * @param type $post
     * @return type
     */
    ...
    public function carregaUsuariosSolicitante($idusuario, $idEmpresa)
    {
        ...
        if($sql->rowCount() > 0):           
            ...
            elseif($gestor[0]['gestor_usuario'] == 'S'):
                $sqlUsers = "
                    SELECT
                        u.z_sga_usuarios_id AS idUsuario,
                        u.nome_usuario,
   ...
   public function carregaAbaAnonizadosFluxoByGrupo($search, $orderColumn, $orderDir, $offset, $limit, $grupos, $idEmpresa)
     ...
     * @return type
     */
    public function dadosGestorGrupos($idProgramas, $idUsuario, $idEmpresa)
    {
        try{
            $sql = "
                SELECT
